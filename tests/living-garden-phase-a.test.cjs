...
test('migrates only garden state once, keeping learning and test data byte-for-byte',()=>{
 ...
 assert.equal(state.garden.livingGardenVersion,1);assert.equal(state.garden.growth,0);assert.deepEqual(JSON.parse(JSON.stringify(state.garden.locations)),{bunny:'garden',cat:'garden',bird:'garden'});
...
test('environment is naturally bounded, lightweight and honors reduced motion',()=>{
 ...
 assert(page.indexOf('living-garden.js?v=20260924-b')<page.indexOf('app.js?v=20260924-living-b'));
});
