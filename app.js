...
function defaultState(){return{version:3,xp:0,week:{id:'2026-09-17-unit-5a',title:'Sept 17 · Unit 5A',ids:[],focusIds:[]},myWords:{},pastWeeks:[],learningGroupsVersion:1,lib:{},garden:{growth:0,pos:{},bunnySeated:false,stored:[]},settings:{sound:true,music:true,musicV2:true,voice:'',audioDefaultV17:true},stats:{answers:0,sessions:0},recentWords:[]}}
...
function normalizeWord(raw={},old=null){
  ...
  return{id:word,word,
    ...
    pronunciationSource:raw.pronunciationSource??old?.pronunciationSource??'',audioTried:old?.audioTried||raw.audioTried||false,parentPriority:!!(raw.parentPriority??old?.parentPriority??false),focusHistory:!!(raw.focusHistory??old?.focusHistory??false),autoFillSources:raw.autoFillSources??old?.autoFillSources??{},learn};
}
...
function treasureStatusText(){if(window.LivingGarden)return `🌿 ${state.garden.growth} growth moments`;const t=treasureProgress();return t.done?'🎁 Collection complete':`🎁 ${t.away} ${t.away===1?'word':'words'} to treasure`}
function renderTopbar(){
  if(window.LivingGarden){const label=$('#levelLabel'),count=$('#xpLabel'),fill=$('#xpFill');if(label)label.textContent='🌿 Living Garden';if(count)count.textContent=`${state.garden.growth} words grown`;if(fill)fill.style.width=`${Math.min(100,state.garden.growth%4*25)}%`;const music=$('#musicToggle'),enabled=!!(state.settings.music&&state.settings.sound);if(music){music.textContent='♫';music.classList.toggle('off',!enabled);music.setAttribute('aria-label',enabled?'Mute music and sound effects':'Turn music and sound effects on');music.title=enabled?'Music + sound effects on':'Music + sound effects off'}return}
...
function practiceIds(){return uniqueWordIds([...state.week.ids,...Object.keys(state.myWords||{})]).filter(id=>!!state.lib[id])}
...
function right(w,q){
  ...
  if(q.stage<4){save();renderTask();const stage=q.stage;setTimeout(()=>{if(!session?.q||session.q.id!==w.id||session.q.stage!==stage)return;const next=newQuestion(w,stage+1,q.range);next.practiceKind=q.practiceKind;next.myWordTrial=false;session.q=next;helpKind='';renderTask()},520);return}
  ...
  const unlockedReward=window.LivingGarden?null:rewards.find(r=>r.id!=='bunny'&&beforeXp<r.xp&&state.xp>=r.xp)||null;
  if(unlockedReward&&!state.garden.stored.includes(unlockedReward.id))state.garden.stored.push(unlockedReward.id);
  const livingMoment=window.LivingGarden?.onWordComplete(state.garden);
  const finished=session.count>=session.goal;gardenCelebration={moment:livingMoment?.moment||'',word:w.word,emoji:w.pictureEmoji||'🌱',gain,beforeGrowth,growth:state.garden.growth,finished,askFeeling,unlock:unlockedReward?{id:unlockedReward.id,label:unlockedReward.label,icon:unlockedReward.icon}:null};session.q=null;save();
...
function pastGroupsHtml(){const weeks=[...(state.pastWeeks||[])].reverse();if(!weeks.length)return'<p class="word-group-note">Past Tests will appear here automatically when you import the next week. Old ★ focus is kept as history, but it will not affect current Play.</p>';return weeks.map(week=>{const ids=uniqueWordIds(week.ids).filter(id=>state.lib[id]);return`<div class="past-week-block"><div class="past-week-head"><div><h3>${esc(week.title||'Past test')}</h3><small>${ids.length} saved words${week.archivedAt?` · archived ${esc(week.archivedAt)}`:''}</small></div><button type="button" class="auto-fill-btn" data-fill-past="${esc(week.id)}" ${autoFillBusy?'disabled':''}>✨ Fill Missing Info</button><button class="past-review-btn" data-past-review="${esc(week.id)}">Review up to 5 →</button></div><div class="past-word-chips">${ids.map(id=>{const w=state.lib[id],focus=(week.focusIds||[]).includes(id)||w.focusHistory,mine=!!myWordMeta(id);return`<button type="button" class="past-word-chip ${focus?'was-focus':''} ${mine?'is-myword':''}" data-past-word="${id}" title="${mine?'Also in My Words':'Past test word'}">${esc(w.word)}${mine?' · 📚':''}</button>`}).join('')}</div></div>`}).join('')}
function wordGroupsHtml(){const week=state.week.ids.map(id=>state.lib[id]).filter(Boolean),mine=Object.keys(state.myWords||{}).map(id=>state.lib[id]).filter(Boolean),pastCount=new Set((state.pastWeeks||[]).flatMap(x=>x.ids||[])).size;return`<section class="panel word-groups-panel"><div class="panel-head"><div><p class="eyebrow">WORD GROUPS</p><h2>今の目的で分けて見る</h2><p>This Weekは短期、My Wordsは日常で使う長期、Past Testsは必要な時だけ復習。</p></div></div><div class="word-groups-summary"><div class="word-group-summary week"><small>THIS WEEK</small><b>${week.length}</b><small>今週のテスト。★はこの週だけ有効。</small></div><div class="word-group-summary mine"><small>MY WORDS</small><b>${mine.length}</b><small>Journalなどで見つけた、長く覚えたい言葉。</small></div><div class="word-group-summary past"><small>PAST TESTS</small><b>${pastCount}</b><small>通常Playにはほぼ出さず、Review用。</small></div></div><div class="word-group-tabs"><button class="word-group-tab ${parentGroupTab==='week'?'active':''}" data-group-tab="week">📝 This Week</button><button class="word-group-tab ${parentGroupTab==='my'?'active':''}" data-group-tab="my">📚 My Words</button><button class="word-group-tab ${parentGroupTab==='past'?'active':''}" data-group-tab="past">🌿 Past Tests</button></div><p id="autoFillStatus" class="auto-fill-notice" role="status" aria-live="polite">${esc(autoFillNotice)}</p><div class="word-group-pane ${parentGroupTab==='week'?'active':''}" data-group-pane="week"><div class="auto-fill-toolbar"><button type="button" class="auto-fill-btn" data-fill-group="week" ${autoFillBusy?'disabled':''}>✨ Fill Missing Info · This Week</button><small>学校の英語の意味は変更しません。空欄と未登録音声だけ検索。</small></div><p class="word-group-note"><b>${esc(state.week.title)}</b> · ★は今週の中だけで出題を優先。週が変わると履歴だけ残ります。</p><div class="group-word-grid">${week.map(thisWeekGroupCard).join('')||'<p>No words yet.</p>'}</div></div><div class="word-group-pane ${parentGroupTab==='my'?'active':''}" data-group-pane="my"><div class="auto-fill-toolbar"><button type="button" class="auto-fill-btn" data-fill-group="my" ${autoFillBusy?'disabled':''}>✨ Fill Missing Info · My Words</button><small>既存のMy Wordsも一括補完。登録時は自動検索します。</small></div><p class="word-group-note">日常で書けるようになりたい言葉。PlayではまずStep 4で記憶を確認し、必要な時だけStep 3に戻ります。</p><div class="group-word-grid">${mine.map(myWordGroupCard).join('')||'<p>まだMy Wordsはありません。上の「+ Add My Word」からJournalの言葉を追加できます。</p>'}</div></div><div class="word-group-pane ${parentGroupTab==='past'?'active':''}" data-group-pane="past">${pastGroupsHtml()}</div></section>`}
