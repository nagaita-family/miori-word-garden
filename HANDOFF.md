...
## Garden placement precision, v51, 2026-09-24

**Issue and fix:** Character dragging used a 12 px margin outside every item's bounding rectangle to decide reactions. Placing a character near a house or item could therefore turn into an unexpected interaction. Item overlap also made the lower item hard to grab directly. Drop detection now prefers the visually topmost item under the pointer and only falls back to actual item rectangles, without the proximity margin. Drag & Drop reactions still work when dropped on an item.

**Precise alternative:** Tap a Garden character → Move freely, or tap any placed item, tree or house → Move / Move house / Move tree, then tap the intended point in the Garden. That action sets the saved position even when another item is under the tap, with a visible placement outline and Cancel control. Direct dragging remains available. Existing character locations, Garage unlock/storage and learning data are unchanged; no Garden migration.

**Validation and publication:** Targeted C.6/D.2/D.4/placement tests 15/15 and full Garden/Play/Weekly/Parent regression 57/57, including a character release just outside an item and tap placement over an overlapping item. Syntax and whitespace checks passed. Implementation commit `07d0109e08973d8a48c9727f1f3eeb453fc08742` deployed in successful Pages run `36021522430`. Production browser loaded v51: Move house placed Main House at the selected point; Move freely placed Bunny on Bird Nest House without an unintended entry or reaction; a subsequent direct House drag and page reload preserved both positions. The overlap itself is intentionally allowed, and the selected-item/tap placement route avoids the grab conflict. Physical iPad/Pencil still needs device QA. The local checkout is still behind remote main and contains published work as uncommitted changes; preserve it.

## Garden direct Drag & Drop precision, v52, 2026-09-25

User's 11-second Bunny recording after v51 showed the Bunny visibly trailing the pointer while dragged, although Move freely worked. Direct dragging now starts from the character's rendered center and retains the pointer's original grip offset. This matters when the Bunny is already moving under its idle/action transition or is grabbed away from its center. Pointer movement uses the newest coalesced coordinate if available; releasing applies the pointerup coordinates before either saving an empty-ground position or invoking the item reaction. The same final-release correction applies to draggable items and houses. Existing overlapping-item hit ordering, Garage, Look Inside and saved Garden positions remain in place. No learning state or migration changes.

Validation: a C.6 behavior test covers an off-center grab and a pointerup farther than the last pointermove for Bunny and Bench; `node --test tests/*.test.cjs` passes 57/57, with syntax and whitespace checks passing. Desktop live drag and Apple Pencil device behavior should be checked after deployment; simulated pointer tests cannot prove Pencil hardware timing. The local checkout remains behind GitHub main with published work as uncommitted snapshots; do not reset it.

Publication and live check: implementation commit `34de2a644ab308e912b22a7f9a6c73efb4f0c1e5` deployed in successful Pages run `36060952797`. Production loaded the v52 badge and `living-garden.js?v=20260925-drag`. In desktop Chrome, Bunny was grabbed off-center (pointer x619 versus visible center around x632), dragged to pointer x782/y699, and released with Bunny's center around x768/y699: the original horizontal grip difference was preserved. Saved position `left:57.0123%;top:83.0467%` survived reload. This verifies a mouse path; user should check actual Apple Pencil drag on iPad because its event cadence can differ.
