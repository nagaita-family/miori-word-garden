...
**Validation:** D.3b state test checks saved, bounded independent room positions and locked/outside residents; D.2 and D.3 targeted tests 7/7, syntax and whitespace checks pass. Publication and browser QA are recorded after deployment.

**Current:** D.3b implementation ready for publication. **Next:** D.4 Friend Houses, in its own complete release. Current local checkout remains behind remote main as noted above; preserve all uncommitted published code while reconciling.
