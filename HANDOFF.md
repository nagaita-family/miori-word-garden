...
Verification/deployment checkpoint: full regression 37/37 on the initial C.6 implementation; targeted Garden suite 19/19 after the visual and drag fixes. Browser QA on the published desktop Garden confirmed v44 assets, only Tree/House and Bunny/Bird at 0 XP, smaller landmark proportions, Bunny drag into House, visible reaction/return control, and moved Tree position surviving reload. The item drag preserves the pointer's grab offset; a targeted pointer test covers this. An invisible disabled fruit button intercepted grabs on part of the Tree, so disabled fruit now ignores pointer input. The seven XP items and Cat were checked through state/render/reaction tests, not an earned-content browser session. Actual iPad Safari / Pencil and narrow viewport remain device checks. Initial C.6 run 35965622110, visual fix 35966851519, and pointer-offset run 35967082707 all deployed successfully; disabled-fruit fix awaits its Pages check.
