...
**Tests:** D.2 tests cover timed entry/exit and door classes, occupancy faces, chosen and group exits, location persistence, indoor interaction blocking, unchanged learning and reduced-motion styling. Targeted D.2/B/D.0/D.1 tests 16/16; major Garden/Play/Weekly/Parent regression 47/47; `node --check living-garden.js` and `git diff --check` passed. Browser interaction QA, commit and Pages status follow after publication. Actual iPad Safari/Pencil and narrow viewport remain device checks.

**Current:** main, D.2 implementation prepared. **Next:** D.3 Look Inside / single dollhouse room, then D.4 Friend Houses. No interior room or friend cottage has begun.
