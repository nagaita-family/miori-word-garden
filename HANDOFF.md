...
**Validation:** D.4 tests cover house unlock visibility, permanent movement, location persistence, indoor Garden reaction block, individual and group door exit, separate house occupancy and Main House room filtering. D.4 targeted 4/4 and full Garden/Play/Weekly/Parent regression 55/55; syntax and whitespace checks passed. Browser QA, implementation commit and Pages run will be appended after deployment. iPad Safari/Pencil and narrow viewport require actual device verification.

**Current:** D.4 implemented, publication pending. Next phase should be chosen after user feedback. Local checkout still has uncommitted published snapshots because GitHub git fetch timed out; GitHub main is authoritative and local work must be preserved.
