...
**Tests:** D.3 cases verify location-filtered residents, locked Cat, persistent location after reload, room closing and selected exit through the existing door, reused art and reduced motion. Targeted D.0–D.3 checks 12/12; full Garden/Play/Weekly/Parent suite 50/50; `node --check living-garden.js` and `git diff --check` passed. Browser QA and GitHub Pages publication are recorded after deployment. Actual iPad Safari/Pencil and narrow viewport remain device checks.

**Current:** D.3 implementation complete locally, publication pending. **Next:** D.4 Friend Houses; D.3 has one room for Main House only. Keep the existing exterior item movement, Garden Drag & Drop, learning data and Garage intact.
