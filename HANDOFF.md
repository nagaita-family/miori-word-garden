...
**Tests:** `node --check living-garden.js`; relevant D.0, C.6 and Phase B tests pass; one major regression run passed 41/41 (Garden, Play, Weekly, Parent). Browser QA, commit and Pages result to be appended after publication. Actual iPad Safari/Pencil, touch hit areas, reduced-motion device setting and narrow viewport remain unverified on a physical device.

**Current:** main; D.0 implementation prepared. **Next:** D.1 Garden Garage, then D.2 House occupancy/door, D.3 Look Inside, D.4 Friend Houses. Garage still uses the C.6 shelf as interim storage; existing House location and return control remain, with no indoor scene. No D.1–D.4 work begun in this checkpoint.
